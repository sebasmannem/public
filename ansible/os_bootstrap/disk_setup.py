...
def blkid2dict(blkid):
  ...
  for pair in splitter_re(blkid):
    key, val = par.split('=', 1)
    if val[0] == '"' and val[-1] == '"':
      val = val[1:-1]
    ret[key] = val
  return retblkid2dict
...
if __name__ == "__main__":
  ...
  if not args.info:
    ...
    for d in valid_disks:
      try:
        subprocess.check_call(['parted', '-s', '-m', '/dev/{0}'.format(d), '--','mklabel',args.pttable,'mkpart','lvm','1','-1'], stdout=devnull, stdin=devnull, stderr=devnull)
        part='/dev/{0}1'.format(d)
        parts.append(part)
        subprocess.check_call(['pvcreate', part.format(d)], stdout=devnull, stdin=devnull, stderr=devnull)
      except:
        print('Failed to create new partition on {0} or pv on new partition.'.format(d))
        exit(1)
    ...
    if args.swap:
      try:
        subprocess.check_call(['lvcreate', '-y', '-n', 'lv_swap', '-L', '500M', '-i', len(parts), args.vg]+parts, stdout=devnull, stdin=devnull, stderr=devnull)
        subprocess.check_call(['mkswap', '/dev/{0}/lv_swap'.format(args.vg)]+parts, stdout=devnull, stdin=devnull, stderr=devnull)
        subprocess.check_call(['swapon', '/dev/{0}/lv_swap'.format(args.vg)]+parts, stdout=devnull, stdin=devnull, stderr=devnull)
      except:
        print('Tried to create, initiate and use swap /dev/{0}/lv_swap, but failed. Invalid VG name?'.format(args.vg))
        os.exit(1)
    ...
    if args.root:
      ...
      for pv in parts:
        ...
        cryptsetup.communicate('wachtwoord')
      ...
      try:
        ...
        for line in btrfs.stdout:
          if line[-1] == '\n':
            line=line[:-1]
          ID, gen, top level, path = splitter_re.split(line)
          if path=='__active':
            subprocess.check_call(['btrfs', 'subvolume', 'set-default', ID, '/mnt/gentoo'], stdout=devnull, stdin=devnull, stderr=devnull)
            break
      except:
        print('Tried to create root btrfs filesystem, but failed. Please investigate..'.format(args.vg))
      try:
